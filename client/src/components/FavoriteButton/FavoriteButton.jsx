...
const FavoriteButton = ({ bookId, small = false }) => {
  ...
  // 🛡 Захист: не рендеримо, поки favorites ще не підтягнулись
  ...
  return (
    <button
      ...
      title={isFavorite ? t("book.unfavorite") : t("book.favorite")}
      disabled={isLoading}
    >
      <AnimatePresence mode="wait" initial={false}>
        <motion.div
      ...
      {!small && (
        <span>{isFavorite ? t("book.unfavorite") : t("book.favorite")}</span>
      )}
    </button>
  );
};
