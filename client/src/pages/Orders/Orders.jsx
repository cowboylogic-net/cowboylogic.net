...
const Orders = () => {
  ...
  return (
    <div className={styles.ordersPage}>
      ...
      {!loading && !error && orders.length === 0 && <p>{t("orders.noOrders")}</p>}
      ...
      {!loading && !error && orders.length > 0 && (
        orders.map((order) => (
          <div key={order.id} className={styles.orderCard}>
            ...
            <p>{t("orders.total")}: ${order.totalPrice.toFixed(2)}</p>
            <ul className={styles.orderList}>
              {order.OrderItems.map((item) => (
                <li key={item.id}>
                  <strong>{item.Book.title}</strong> — {item.quantity} {t("orders.pcs")} @ ${item.price}
                </li>
              ))}
            </ul>
            {user?.role === "admin" && (
