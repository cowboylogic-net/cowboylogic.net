...
export default function verifySquareSignature(req, res, next) {
  try {
    ...
    const signatureHeader = (req.get("x-square-hmacsha256-signature") || "").trim();
    if (!signatureHeader) {
    ...
    const rawBodyStr = Buffer.isBuffer(req.body) ? req.body.toString("utf8") : String(req.body || "");

    const base = notifUrl + rawBodyStr;

    const computed = crypto.createHmac("sha256", secret).update(base).digest("base64");
